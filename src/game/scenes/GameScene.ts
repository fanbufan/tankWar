...
import { reduceKeysToActions, reduceKeysToPlayerOneActions, reduceKeysToPlayerTwoActions } from "../input";
import { createRuntime, getRuntimeSnapshot, stepRuntime, toggleRuntimePause, type Runtime } from "../runtime";
import { tankTint } from "../tankAppearance";
import type {
...
import type { EnemyType, GridPoint, LevelDefinition, PowerUpType, TileType } from "../types";
...
export class GameScene extends Phaser.Scene {
  ...
  private arcadeAudio = new ArcadeAudio();
  ...
  public preload(): void {
    if (!this.textures.exists(SPRITE_SHEETS.tanks.key)) {
    ...
    if (!this.textures.exists(SPRITE_SHEETS.effects.key)) {
    ...
    if (!this.textures.exists(SPRITE_SHEETS.powerups.key)) {
  ...
  public create(): void {
    this.runtime = createRuntime(this.levelIndex, this.startingScore, { twoPlayers: this.players === 2, customLevel: this.customLevel });
    this.keysDown.clear();
  ...
  public update(_time: number, delta: number): void {
    ...
    const input =
      this.players === 2
        ? {
            player1: { ...reduceKeysToPlayerOneActions(this.keysDown), pause: false },
            player2: { ...reduceKeysToPlayerTwoActions(this.keysDown), pause: false },
          }
        : { ...reduceKeysToActions(this.keysDown), pause: false };
    const events = stepRuntime(this.runtime, input, delta);
    this.arcadeAudio.playRuntimeEvents(events);
  ...
  private createHudTexts(): HudTexts {
    ...
    return {
      ...
      livesP2: this.add.text(panelX + 54, 278, "", textStyle).setOrigin(0.5, 0).setDepth(10),
      enemies: this.add.text(panelX, 0, "", textStyle).setVisible(false).setDepth(10),
      score: this.add.text(panelX + 10, 392, "", { ...textStyle, fontSize: "10px" }).setDepth(10),
      power: this.add.text(panelX + 10, 180, "", { ...textStyle, fontSize: "10px" }).setDepth(10),
      powerP2: this.add.text(panelX + 10, 194, "", { ...textStyle, fontSize: "10px" }).setDepth(10),
      item: this.add.text(panelX + 10, 364, "", { ...textStyle, fontSize: "10px" }).setDepth(10),
  ...
  private maybeTransition(snapshot: RuntimeSnapshot): void {
    ...
    this.arcadeAudio.playOutcome(snapshot.status);
  ...
  private render(snapshot: RuntimeSnapshot): void {
    ...
    if (snapshot.player2) {
      this.drawTank(snapshot.player2, snapshot.elapsedMs);
    }
  ...
  private drawTank(tank: RuntimeTankSnapshot, elapsedMs: number): void {
    if (tank.owner === "player" && tank.lives <= 0) {
      return;
    }
  ...
  private drawHudPanel(snapshot: RuntimeSnapshot): void {
    ...
    if (snapshot.player2) {
      this.drawMiniTank(panelX + 26, 276, NES_COLORS.hudDark);
    }
  ...
  private updateHudText(snapshot: RuntimeSnapshot): void {
    this.hud.stage.setText(snapshot.hud.isCustomStage ? "ED" : `${snapshot.levelIndex + 1}`);
    this.hud.lives.setText(`${snapshot.hud.lives}`);
    this.hud.livesP2.setText(snapshot.hud.livesP2 === undefined ? "" : `${snapshot.hud.livesP2}`);
    this.hud.help.setText(snapshot.player2 ? "1P\n\n2P" : "1P");
    this.hud.score.setText(`${snapshot.hud.score.toString().padStart(6, "0")}`);
    this.hud.power.setText(`PWR ${snapshot.hud.power}`);
    this.hud.powerP2.setText(snapshot.hud.powerP2 === undefined ? "" : `P2  ${snapshot.hud.powerP2}`);
    this.hud.item.setText(snapshot.hud.item ? `ITM ${this.itemLabel(snapshot.hud.item)}` : "ITM --");
  }

  private itemLabel(item: PowerUpType): string {
